...
class City():
    ...
    def __init__(self, name: str, coordinates: Tuple[float, float], city_type: str,\
        ...
        :return: None
        """
        # initialises all variables needed to create a city instance variable
        self.name = name
        ...
        #TODO
        # creates new instance variables and stores the city id as the key and the instance variable as the value
        City.id_to_cities[self.city_id] = self

        # checks if there name of the city is the same. if so then add to the list.
        if self.name in self.name_to_cities:

            # if city name key already exists, the new instance variable is appended to the city class instance 
            City.name_to_cities[self.name].append(self)
        else:

            # if city name doesn't exist it creates one. 
    ...
    def distance(self, other_city: City) -> int:
        ...
        """
        #TODO
        # get coordinates and program the distance using the great circle method. 
        coordinates = self.coordinates
        other_city_coord = other_city.coordinates 

        # calculates the distance using the great circle method
        dist = math.ceil(geopy.distance.great_circle(coordinates, other_city_coord).km)        
    ...
    def __str__(self) -> str:
        ...
        """
        # stores the strong in a vairable containing the city name + city id.
        result = f"{self.name} ({self.city_id})"
    ...
    def get_table_data(self) -> list[str]:
        ...
        """
        # adds all items into a list and converts item into a string
        value = [str(self.name), str(self.coordinates) , str(self.city_type) , str(self.population) , str(self.city_id)]
...
def get_city_by_id(city_id: int) -> City | None:
    ...
    # returms the value of the city object
...
def get_cities_by_name(city_name: str) -> list[City]:
    ...
    """
    #TODO
    # returns the object of the city class
