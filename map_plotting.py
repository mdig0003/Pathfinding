...
def plot_itinerary(itinerary: Itinerary, projection = 'robin', line_width=2, colour='b') -> None:
    ...
    # gets the cities in a list
    cities = itinerary.cities

    # creates the map
    m = Basemap(projection=projection, lon_0=-5)
    m.drawcoastlines()

    # prints colour of map
    m.drawmapboundary(fill_color='aqua')
    m.fillcontinents(color='coral', lake_color='aqua')
    
    # plots coordinates, stores each value of x and y into a list
    x, y = m([c.coordinates[1] for c in cities], [c.coordinates[0] for c in cities])

    # plots the different points in the basemap plot
    ...
    # saves the figure and shows the plot
